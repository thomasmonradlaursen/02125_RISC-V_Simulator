...
pub fn access_memory(mem: &mut [u8], computation: &Computation, control: &Control) -> MemoryResult {
    ...
    if issue_warning(mem, control, computation) {
...
fn issue_warning(mem: &mut [u8], control: &Control, computation: &Computation) -> bool {
    let mut res = false;
    if computation.result < 0 || computation.result > mem.len() as i32 {
        if control.mem_read {
            ...
            message.push_str("Memory cannot be access due to this, and the simulator will continue as if nothing happened.");
