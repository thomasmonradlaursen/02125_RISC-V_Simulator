...
pub fn simple_pipeline() -> Vec<f32> {
    ...
    let if_id: Vec<f32> = components::register(220.0, 150.0, width, height);
    let id_ex: Vec<f32> = components::register(450.0, 150.0, width, height);
    let ex_mem: Vec<f32> = components::register(690.0, 150.0, width, height);
    let mem_wb: Vec<f32> = components::register(920.0, 150.0, width, height);
    ...
    let mux_wb: Vec<f32> = components::multiplexer(1000.0, 240.0, width, height);
    ...
    let wire_imm_rs2_1: Vec<f32> = components::wire((470.0, 175.0), (540.0, 175.0), width, height);
    let wire_imm_rs2_2: Vec<f32> = components::wire((540.0, 175.0), (540.0, 210.0), width, height);
    let wire_imm_rs2_3: Vec<f32> = components::wire((540.0, 210.0), (570.0, 210.0), width, height);
    ...
    let wire_mux_reg_1: Vec<f32> =
        components::wire((1020.0, 275.0), (1040.0, 275.0), width, height);
    let wire_mux_reg_2: Vec<f32> =
        components::wire((1040.0, 275.0), (1040.0, 120.0), width, height);
    let wire_mux_reg_3: Vec<f32> = components::wire((1040.0, 120.0), (250.0, 120.0), width, height);
    let wire_mux_reg_4: Vec<f32> = components::wire((250.0, 120.0), (250.0, 240.0), width, height);
